"""KR2 schema check: every Done run-log entry must carry a non-null outcome_verdict.

A Done entry without ``outcome_verdict`` is uninspectable — the operator cannot
assess correctness. Null verdicts on non-Done entries are fine: a halt or error
explains the missing assessment via ``halt_reason``.

Usage::

    python -m drain_cycle.kr2_check <run-log.json> [...]

Exits 0 when all Done entries in every file carry outcome_verdict.
Exits 1 when any Done entry lacks outcome_verdict.
